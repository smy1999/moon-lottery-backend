...
public class RefuseState extends AbstractState {
    ...
    public Result revoke(Long activityId, Constants.ActivityState current) {
        ...
        return Result.buildResult(Constants.ResponseCode.SUCCESS.getCode(), "拒绝活动撤回成功");
    }
    ...
    public Result close(Long activityId, Constants.ActivityState current) {
        ...
        return Result.buildResult(Constants.ResponseCode.SUCCESS.getCode(), "拒绝活动关闭成功");
    }

    @Override
    public Result open(Long activityId, Constants.ActivityState current) {
        return Result.buildErrorResult("拒绝活动不能开启");
    }

    @Override
    public Result execute(Long activityId, Constants.ActivityState current) {
        return Result.buildErrorResult("拒绝活动不能执行");
    }
}
